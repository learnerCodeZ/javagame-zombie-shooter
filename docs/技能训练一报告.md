...
| | F4.3 | 我的记录 | 展示当前登录用户的全部战绩（按时间倒序、含难度标注） |
...
- **启动模块**（`GameApp`）：程序入口，初始化全局暗色主题，在 EDT 上打开登录窗。
- **用户模块**（`LoginFrame` / `RegisterFrame`）：手机号注册、登录（密码 MD5 存储），含忘记密码入口。
- **菜单模块**（`MainFrame`）：主菜单与功能跳转；按角色控制「用户管理」按钮可见性（仅 admin），开始游戏前选难度。
- **游戏模块**（`GameWindow` / `GamePanel` / `GameController` / `Player` / `Zombie` / `Bullet` / `Difficulty`）：核心玩法。`Timer` 驱动主循环，`GamePanel` 负责渲染与输入，`GameController` 负责刷怪、碰撞、计分；三类对象实现 `GameObject` 接口（多态），`Difficulty` 枚举提供两套难度参数。
- **排行榜模块**（`LeaderboardFrame`）：按难度分简单榜 / 困难榜，外加「我的记录」。
- **账号安全模块**（`AdminFrame` / `ChangePasswordDialog`）：管理员审核重置、删除用户（admin 受保护、操作包事务）；修改密码。
- **数据访问模块**（`DBUtil` / `UserDao` / `RecordDao` / `ResetRequestDao`）：封装全部 SQL，统一 `PreparedStatement` 防注入、事务保证原子性。
...
- **登录界面**：手机号框（限 11 位数字）/ 密码 / 登录 / 去注册 / 忘记密码；回车可登录。
- **注册界面**：手机号 / 密码 / 昵称 / 注册 / 返回；含输入校验提示。
- **主菜单**：开始游戏 / 排行榜 / 修改密码 / 用户管理(admin) / 说明 / 设置 / 退出；暗色大按钮纵排。
- **游戏界面**：游戏画布 + 顶部 HUD（分数/血量/击杀/时间/难度）；鼠标瞄准、左键射击。
- **结算界面**：分数/击杀/用时 + 再来一局 / 排行榜 / 主菜单；血量归零弹出。
- **排行榜界面**：JTable（排名/昵称/分数/难度/击杀/存活/时间）；简单榜/困难榜（按分数）+ 我的记录（全部、按时间）。
- **用户管理界面（admin）**：上为所有用户表、下为待审申请表，配通过/拒绝/删除/刷新/返回；admin 专用。
...
**排行榜**：进入后可切「简单榜 / 困难榜」（全部玩家、按分数），或点「我的记录」看自己的全部战绩（按时间倒序、每条标注难度）。
