...
| `phone` | VARCHAR(11) | NOT NULL, UNIQUE | — | 登录手机号(11位)，全局唯一 |
...
| `difficulty` | VARCHAR(8) | NOT NULL | 'EASY' | 难度：`EASY`（简单）/ `HARD`（困难） |
| `record_time` | DATETIME | NOT NULL | CURRENT_TIMESTAMP | 记录时间 |

- **主键**：`id`
- **外键**：`fk_record_user (user_id) → user(id)`
- **索引**：`idx_user (user_id)` —— 按"我的记录"查询用；`idx_score (score)` —— 排行榜按分数排序用；`idx_diff_score (difficulty, score)` —— 分难度排行榜(先按难度过滤再按分数排序)用
...
| 唯一键 | `uk_phone` | `user.phone` | 手机号不可重复 |
...
| 索引 | `idx_diff_score` | `game_record.(difficulty, score)` | 加速分难度排行榜(先按难度过滤再按分数排序) |
...
| 非空 | NOT NULL | 各核心字段 | `phone/password/role/user_id/score/status` 等不能为空 |
...
| phone | VARCHAR | 11 | 是 | 是 | 登录手机号 | 11 位数字 |
...
| difficulty | VARCHAR | 8 | 是 | 否 | 难度 | `EASY`（简单）/ `HARD`（困难），默认 `EASY` |
...
> `PasswordResetRequest` 模型另含 `phone`、`nickname` 两个仅显示用的字段，由 DAO JOIN `user` 表填充，**不是本表的列**。
...
| id | phone | password(MD5) | nickname | role |
|---|---|---|---|---|
| 1 | 00000000000 | e10adc…883e（=123456） | 管理员 | admin |
| 2 | 13800000001 | 同上 | 张三 | user |
| 3 | 13800000002 | 同上 | 李四 | user |
...
| DAO 方法 | SQL |
|---|---|
| `UserDao.register` | `INSERT INTO user(phone,password,nickname) VALUES(?,?,?)`（`role` 取默认值 `user`，普通用户注册直接成功） |
| `UserDao.login` | `SELECT * FROM user WHERE phone=? AND password=?` |
| `UserDao.findByPhone` | `SELECT COUNT(*) FROM user WHERE phone=?` |
...
| `RecordDao.mine` | `SELECT * FROM game_record WHERE user_id=? ORDER BY record_time DESC` |
...
| `ResetRequestDao.listPending` | `SELECT r.*, u.phone, u.nickname FROM password_reset_request r JOIN user u ON r.user_id=u.id WHERE r.status='pending' ORDER BY r.request_time` |
