...
classDiagram
    ...
    namespace dao {
        ...
        class UserDao {
            ...
            +login(String phone, String pwd) User
            +findByPhone(String phone) boolean
        }
        class RecordDao {
            +saveRecord(GameRecord r) boolean
            +topN(int n, String difficulty) List~GameRecord~
...
```mermaid
flowchart TD
    A(["点击 排行榜"]) --> B{"选 简单榜 / 困难榜 / 我的记录?"}
    B -->|简单榜/困难榜| C["topN(N, 难度) 按分数倒序"]
    B -->|我的记录| D["mine(用户) 全部战绩 按时间倒序"]
    C --> E["填入 JTable(含难度列)展示"]
    D --> E
    E --> F(["完成"])
```
...
sequenceDiagram
    ...
    U->>L: 输入手机号/密码，点击登录
    L->>UD: login(phone, pwd)
    UD->>DB: SELECT * FROM user WHERE phone=? AND password=MD5(?)
    DB-->>UD: 用户记录 / 空
    alt 密码匹配
        UD-->>L: User 对象
        L->>L: 记录当前登录用户
...
| 排行榜界面 | JTable：排名/昵称/分数/难度/击杀/用时/日期 + 切换按钮 | 简单榜/困难榜(按分数) + 我的记录(全部、按时间) |
